...
class Edrone():
	...
	def __init__(self):
		...
		#after tuning and computing corresponding PID parameters, the parameters are-
		...
		self.currenttime=0
		self.max_values = [1800,1800,1600,1800]#The max and min values of pitch, roll, throttle and yaw are specified 
		...
		self.yawerr=rospy.Publisher('/yaw_error',Float64,queue_size=1)
                #This makes a publisher for sending an interrupting message to the lua script to calculate and publish the next path point
	...
	def pid(self):
		...
		threshZ=0.5# Threshold value or maximum error acceptable at a point in altitude
		...
		if(self.currenttime-self.lasttime>=self.sample_time):
			...
			self.cmd.rcPitch=int(1500+self.error[0]*self.Kp[0]+self.dif_error[0]*self.Kd[0]/self.sample_time+self.sum_values[0]*self.Ki[0]*self.sample_time)
			self.cmd.rcRoll=int(1500+self.error[1]*self.Kp[1]+self.dif_error[1]*self.Kd[1]/self.sample_time+self.sum_values[1]*self.Ki[1]*self.sample_time)
			self.cmd.rcThrottle=int(1500+self.error[2]*self.Kp[2]+self.dif_error[2]*self.Kd[2]/self.sample_time+self.sum_values[2]*self.Ki[2]*self.sample_time)
			self.cmd.rcYaw=int(1500+self.error[3]*self.Kp[3]+self.dif_error[3]*self.Kd[3]/self.sample_time+self.sum_values[3]*self.Ki[3]*self.sample_time)
		...
		if e_drone.success == 1:
			...
			if e_drone.initial != 0:
				...
				if e_drone.counter==e_drone.pointsInPath:
					errX=e_drone.setpoint[1]-e_drone.initial_waypoint[1]
					errY=e_drone.setpoint[2]-e_drone.initial_waypoint[2]
					errZ=e_drone.setpoint[3]-e_drone.initial_waypoint[3]
